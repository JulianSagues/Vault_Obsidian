...
Disp de bloques son disp que envian y reciben info en bloques (4kb) disco hdd.

  

Disp de caracteres (cinta magnetica).

un reloj no es ninguno de esos dispositivos.

  

Controlador-Controladora → Hardware + Firmware (se encuentra en MorherBoard + Dispositivo)

Driver → Software (Se encuentra en SO)

  

E/S programada → CPU → por interrupciones se maneja

Driver → Software ( se encuentra en SO)

E/S por interrupciones DMA

  

El driver es el intermediario o traductor entre el S.O y el dispositivo.
...
- **Definición:** El controlador DMA toma el control del bus del sistema y transfiere _una ráfaga de múltiples bytes consecutivos_ sin liberar el bus entre cada byte.
...
![[5.3_-_Niveles_de_RAID.pdf]]

![[5.2_-_Mapa_conceptual_ES_2023.docx]]
