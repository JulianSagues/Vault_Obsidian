...
- En Spring Boot, los *endpoints* viven dentro de clases anotadas con `**@RestController**`.
- Esta anotación es fundamental y le dice a Spring que esta clase expondrá *endpoints* HTTP.
- Es un atajo que combina dos anotaciones:
    ...
    2. `**@ResponseBody**`: Esta es la "magia". Le dice a Spring que **convierta automáticamente** lo que sea que el método devuelva (un objeto Java, un `Map`, un `String`) en una respuesta **JSON** y la ponga en el cuerpo (body) de la respuesta HTTP.
...
El video muestra cómo crear dos *endpoints* básicos en un proyecto de Spring Boot (que ya debe tener la dependencia `**spring-boot-starter-web**`).
...
`    @GetMapping("/usuario")
    public Map<String, String> obtenerUsuario() {
        ...
        usuario.put("nombre", "José");
        usuario.put("apellido", "Douglas");
        return usuario; // Spring lo convierte a JSON
    }`
...
- Se aprendió que `@RestController` crea *endpoints* que devuelven JSON.
