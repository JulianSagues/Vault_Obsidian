...
| **PUT** | **Update** (Actualizar) | `@PutMapping` | Reemplaza *completamente* un recurso existente. |
...
Un punto clave es que `PUT` y `DELETE` (y a veces `GET`) necesitan saber *sobre qué recurso específico* actuar. Esto se logra pasando un identificador (como un ID) directamente en la URL.
...
En el video, se crea un controlador con cuatro *endpoints* para probar cada método.
...
2. **Prueba POST**
    ...
    - *(Nota: En este ejemplo básico aún no se envía un "body" o cuerpo con datos, eso se verá en el próximo video).*
3. **Prueba PUT (con Path Variable)**
    - **Acción:** `PUT`
...
4. **Prueba DELETE (con Path Variable)**
    - **Acción:** `DELETE`
    - **URL:** `http://localhost:8080/api/ejemplo/5` (El `5` es el ID).
...
- **Próximo Video:** El paso clave que falta: **cómo recibir datos** en los *endpoints* (Path Variables, Parámetros de Consulta y, lo más importante, el cuerpo o **Request Body** en formato JSON) para que `POST` y `PUT` sean realmente útiles.