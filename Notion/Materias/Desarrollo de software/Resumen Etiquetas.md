...
2. **Gestionado (Persistent)**: El objeto está asociado a un contexto de persistencia. JPA rastrea sus cambios y los sincronizará con la base de datos. Se llega a este estado con `em.persist()` o `em.find()`.
3. **Desasociado (Detached)**: El objeto tiene un ID pero ya no está asociado al contexto de persistencia (ej. después de cerrar el `EntityManager`). Los cambios no se sincronizan.
4. **Eliminado (Removed)**: El objeto está marcado para ser eliminado de la base de datos. La eliminación se efectúa al hacer `commit` de la transacción. Se llega a este estado con `em.remove()`.