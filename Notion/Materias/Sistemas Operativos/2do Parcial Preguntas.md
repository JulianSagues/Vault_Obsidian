...
c) EI PIC prioriza todas las interrupciones ya sean enmascarables o no enmascarables.
==d) Todas las anteriores.==
...
2. ¿Cuál es la ventaja de utilizar DMA (acceso directo a memoria) respecto de E/S controlada por interrupciones?
...
3. ¿Qué información no se almacena en el nodo índice?:
a) El propietario del archivo.
==b) El pathname del link si se trata de un link simbólico.
==c) El tipo de archivo.
d) Ninguna de las anteriores.
4. Que afirmación es correcta respecto de un driver:
a) Se ejecuta cuando un dispositivo está utilizando el buffer.
==b) En un sistema monolitico al incorporar un nuevo driver al kernel, éste debe ser recompilado.
==c) Permite la sincronización del shell con el sistema operativo.
d) Ninguna de las anteriores.
5. Sabemos que los sistemas de archivos se identifican con el denominado número mágico. El mismo se almacena en
a) EI MBR
b) El directorio raíz
c) En el sector O del disco.
==d) Ninguna de las anteriores.==
6. Al utilizar nodos índice, la fragmentación interna de un archivo se evita con la/s siguiente/s acciones:
...
c) Formatear los sistemas de archivos con tamaños de bloques pequeños.
==d) Ninguna de las anteriores. ACA TENGO DUDAS==
7. Al utilizar nodos indice, la longitud o tamaño de un archivo
...
==d) Todas las anteriores.==
8. ¿Qué sucede si se produce una interrupción mientras el CPU está atendiendo otra interrupción?
...
9. ¿Qué componente del sistema operativo gestiona la comunicación entre diferentes sistemas de archivos y los dispositivos de almacenamiento?
a) Controlador del dispositivo
==b) Sistema de archivos virtual (VFS)
==c) Tabla de nodos índice
d) Ninguna de las anteriores
10. ¿Cuál es el principal propósito de un sistema de E/S en un sistema operativo moderno?
...
11. Un enlace blando o soft link:
...
12. ¿Qué información se almacena en el nodo Índice?:
a) El número de nodo índice.
b) El tipo de sistema de archivo.
c) Punteros indirectos a bloques de datos.
==d) Todas las anteriores.==
13. Al utilizar 2 discos de 2 Tb. dispuestos en RAID 0, el espacio disponible es de:
a) 2 Tb.
==b) 4 Tb. Para mi es esta, pero se la puso mal, se puede equivocar.
==c) 1.5 Tb.
d) Ninguna de las anteriores
14. ¿Cual afirmación es correcta sobre el mecanismo de interrupciones?
a) El CPU y la controladora de dispositivos deben sincronizarse para realizar la(s) E/S.
b) La controladora de dispositivos (hardware) no interrumpe directamente al CPU.
c) EI PIC prioriza todas las interrupciones ya sean enmascarables o no enmascarables.
==d)Todas las anteriores.==
15. Un driver:
a) Se ejecuta cuando un dispositivo está utilizando el buffer.
b) En un sistema monolitico al incorporar un nuevo driver al Kernel no requiere ser recompilado el mismo.
==c) Sirve en definitiva para comunicar el SO con la controladora del dispositivo
==d) Todas las anteriores.
16. Un enlace duro o hard link:
a Solo permite enlazar archivos del mismo sistema de archivos.
b) Permite enlazar varios archivos.==
==c) Permite enlazar archivos del tipo dispositivos.
==d) Todas las anteriores.==
17. El número mágico que identifica un sistema de archivos se encuentra en:
a) EI MBR.
b) El directorio raíz.
==c) En el Superbloque
==d) Ninguna de las anteriores.
18. ¿Cuál de las siguientes acciones evita la fragmentación interna en archivos?
a) Utilizando herramientas para desfragmentar el disco.
b) Reutilizando los bloques fragmentados con otros archivos.
c) Reutilizando el espacio sobrante dentro del bloque del archivo con datos de otro archivo.
==d) Ninguna de las anteriores.==
19. Que ventaja logra la independencia del dispositivo.
...
d) Ninguna de las anteriores.
20. ¿Cuántas operaciones de disco se necesitan para obtener el nodo-i /home/os/cursos/2k13/parcial.sh?. Suponga resolución completa y que todos los contenidos de los directorios caben en un bloque de disco.
a) 7
==b) 6
==c) 5
d) Ninguna de las anteriores
NO LA SE, NOTEBOOKLM ME TIRA QUE ES NINGUNA
21. Al utilizar tablas de nodos Indice (i-node). El tamaño máximo de un archivo depende de:
a) La cantidad de File Systems que posea el sistema.
b) La controladora del dispositivo.
==c) El diseño del nodo índice.
==d) Ninguna de las anteriores.
